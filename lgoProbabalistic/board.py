...
class GoBoard(object):
    ...
    def reset(self, size):
        ...
        """
        self.board = np.ones((self.maxpoint),dtype=np.int16)*BORDER #This makes square boards full of border
        #self._empty_filling(self.board) #This makes it all empty
        self._linear_filling(self) #This instead makes only the top row empty



    def setSuperko(self,board):
        self.superko.append(board[board.size+2:board.size+2*2].tolist())
    def getSuperko(self):
        return self.superko
    ...
    def _linear_filling(self,board):
        """
        Fills points inside board with EMPTY
        Only fills the top row with empty, unlike _empty_filling above
        Arguments
        ---------
        board : numpy array
            receives a numpy array filled with BORDER

            This *SHOULD* work for any size you want.
        """
        for ind in range(board.size+2,board.size *2+2):
            np.put(board.board,ind, EMPTY)
